					...
					foreach ($products as $key => $product)
					{
						...
						if (isset($customized_datas[$product['id_product']][$product['id_product_attribute']]))
						{
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								...
								<td style="padding: 0.6em 0.4em;width: 30%;"><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.$product['attributes'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').' - '.Tools::displayError('Customized').(!empty($customization_text) ? ' - '.$customization_text : '').'</strong></td>
						...
						if (!$customization_quantity || (int)$product['cart_quantity'] > $customization_quantity)
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								<td style="padding: 0.6em 0.4em;width: 15%;">'.$product['reference'].'</td>
								<td style="padding: 0.6em 0.4em;width: 30%;"><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.$product['attributes'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').'</strong></td>
								<td style="padding: 0.6em 0.4em; width: 20%;">'.Tools::displayPrice(Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt, $this->context->currency, false).'</td>
