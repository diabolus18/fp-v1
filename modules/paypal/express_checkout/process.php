...
class PaypalExpressCheckout extends Paypal
{
	...
	private function setProductsList(&$fields, &$index, &$total)
	{
		...
		$fee = Tools::ps_round((float)PayPal::getCost($this->context->cart), 2);
		
		if($fee > 0) {
			$fields['L_PAYMENTREQUEST_0_NUMBER'.++$index] = (int)1;

			$fields['L_PAYMENTREQUEST_0_NAME'.$index] = $this->l('Paypal fee');
			$fields['L_PAYMENTREQUEST_0_DESC'.$index] = $this->l('Fee applied for paying with Paypal');

			$fields['L_PAYMENTREQUEST_0_AMT'.$index] = Tools::ps_round((float)$fee, $this->decimals);
	...
	public function getTotalPaid()
	{
		...
		//Recargo
		$total = $total + (float)Tools::ps_round((float)PayPal::getCost($this->context->cart), $this->decimals);
