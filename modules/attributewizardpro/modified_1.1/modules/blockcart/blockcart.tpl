	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{/if}{if $product.instructions_valid}_{$product.instructions_valid}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				...
				<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}{if $product.instructions_valid != ''}{if $awp_url_rewrite}?{else}&{/if}ipa={$productAttributeId}&ins={$product.instructions_valid}{/if}" title="{$product.name|escape:htmlall:'UTF-8'}">{$product.name|escape:htmlall:'UTF-8'|truncate:20}</a>
