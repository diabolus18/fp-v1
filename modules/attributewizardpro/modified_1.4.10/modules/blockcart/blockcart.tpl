...
<div id="cart_block" class="block exclusive">
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<dt id="cart_block_product_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{/if}{if $product.instructions_valid}_{$product.instructions_valid}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<span class="quantity-formated"><span class="quantity">{$product.cart_quantity}</span>x</span>
				<a class="cart_block_product_name" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)}{if $product.instructions_valid != ''}{if $awp_url_rewrite}?{else}&{/if}ipa={$productAttributeId}&ins={$product.instructions_valid}{/if}" title="{$product.name|escape:html:'UTF-8'}">
				{$product.name|truncate:13:'...'|escape:html:'UTF-8'}</a>
				<span class="remove_link">{if !isset($customizedDatas.$productId.$productAttributeId)}<a rel="nofollow" class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart.php', true)}?delete=1&amp;id_product={$product.id_product}&amp;special_instructions={$product.instructions_valid|urlencode}&amp;ipa={$product.id_product_attribute}&amp;token={$static_token}" title="{l s='remove this product from my cart' mod='blockcart'}">&nbsp;</a>{/if}</span>
			...
			<dd id="cart_block_combination_of_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{/if}{if $product.instructions_valid}_{$product.instructions_valid}{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
			...
			<dd id="cart_block_instructions_{$product.id_product}_{if $product.id_product_attribute}{$product.id_product_attribute}{/if}{if $product.instructions_valid}_{$product.instructions_valid}{/if}" class="item">
