...
class PaypalPayment extends Paypal
{
	...
	public function getAuthorisation()
	{
		...
		if ($discounts == 0)
		{
			...
			{
				$request .= '&L_NAME'.$i.'='.substr(urlencode($products[$i]['name'].(isset($products[$i]['attributes'])?' - '.$products[$i]['attributes']:'').(isset($products[$i]['instructions'])?' - '.strip_tags($products[$i]['instructions']):'') ),0,127);
		...
		else
		{
			...
			$description = 0;
			for ($i = 0; $i < sizeof($products); $i++)
				$description .= ($description == ''?'':', ').$products[$i]['cart_quantity']." x ".$products[$i]['name'].(isset($products[$i]['attributes'])?' - '.$products[$i]['attributes']:'').(isset($products[$i]['instructions'])?' - '.strip_tags($products[$i]['instructions']):'') ; 
