...
class GCheckout extends PaymentModule
{
    ...
    function preparePayment()
    {
		...
		foreach ($cart->getProducts() AS $product)
			$googleCart->AddItem(new GoogleItem(utf8_decode($product['name'].((isset($product['attributes']) AND !empty($product['attributes'])) ? ' - '.$product['attributes'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '')), utf8_decode($product['description_short']), (int)$product['cart_quantity'], $product['price_wt'], strtoupper(Configuration::get('PS_WEIGHT_UNIT')), (float)$product['weight'])); 
