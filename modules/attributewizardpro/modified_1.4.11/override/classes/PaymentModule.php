				...
				foreach ($products as $key => $product)
				{
					...
					if (isset($customizedDatas[$product['id_product']][$product['id_product_attribute']]))
					{
						...
						'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
							...
							<td style="padding:0.6em 0.4em;"><strong>'.$product['name'].(isset($product['attributes']) ? ' '.$product['attributes'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').' - '.$this->l('Customized').(!empty($customizationText) ? ' - '.$customizationText : '').'</strong></td>
					...
					if (!$customizationQuantity OR (int)$product['cart_quantity'] > $customizationQuantity)
						...
						'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
							<td style="padding: 0.6em 0.4em;">'.((isset($product['reference']) && !empty($product['reference'])) ? $product['reference'] : '&nbsp;').'</td>
							<td style="padding:0.6em 0.4em;"><strong>'.$product['name'].(isset($product['attributes']) ? ' '.$product['attributes'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').'</strong></td>
							<td style="padding: 0.6em 0.4em; text-align: right;">'.Tools::displayPrice(Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt, $currency, false).'</td>
