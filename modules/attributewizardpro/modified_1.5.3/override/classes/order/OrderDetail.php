...
class OrderDetail extends OrderDetailCore
{
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'product_name' => 				array('type' => self::TYPE_HTML, 'validate' => 'isCleanHtml', 'required' => true),
	...
	protected function create(Order $order, Cart $cart, $product, $id_order_state, $id_order_invoice, $use_taxes = true, $id_warehouse = 0)
	{
		...
		$this->product_name = $product['name'].
			((isset($product['attributes']) && $product['attributes'] != null) ?
				' - '.$product['attributes'] : '')
			.((isset($product['instructions']) AND $product['instructions'] != NULL) ? ' - '.$product['instructions'] : '');
