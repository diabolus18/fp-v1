		...
		{if $product.id_product == $lastProductAdded.id_product AND (!$product.id_product_attribute OR ($product.id_product_attribute == $lastProductAdded.id_product_attribute)) AND (!$product.instructions OR ($product.instructions == $lastProductAdded.instructions))}
			<div class="cart_last_product">
				...
				<div class="cart_last_product_content">
					...
					{if isset($product.attributes) && $product.attributes}<a href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'htmlall':'UTF-8'}{if $product.instructions_valid != ''}{if $awp_url_rewrite}?{else}&{/if}ipa={$product.id_product_attribute}&ins={$product.instructions_valid}{/if}">{$product.attributes|escape:'htmlall':'UTF-8'}</a>{/if}
