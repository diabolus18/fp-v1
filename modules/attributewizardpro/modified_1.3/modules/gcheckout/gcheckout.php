...
class GCheckout extends PaymentModule
{
	...
	function hookPayment($params)
	{
		...
		foreach ($params['cart']->getProducts() as $product)
			$googleCart->AddItem(new GoogleItem(utf8_decode($product['name'].(isset($product['instructions']) ? ' '.$product['instructions'] : '')), utf8_decode($product['description_short']), intval($product['cart_quantity']), Tools::convertPrice($product['price_wt'], $currency)));
