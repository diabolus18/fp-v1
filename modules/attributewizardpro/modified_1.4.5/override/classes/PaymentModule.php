			...
			if ($result AND isset($order->id))
			{
				...
				foreach ($products AS $key => $product)
				{
					...
					$query .= '('.(int)($order->id).',
						...
						\''.mysql_real_escape_string($product['name'].((isset($product['attributes']) AND $product['attributes'] != NULL) ? ' - '.$product['attributes'] : '').((isset($product['instructions']) AND $product['instructions'] != NULL) ? ' - '.$product['instructions'] : '')).'\',
					...
					if (isset($customizedDatas[$product['id_product']][$product['id_product_attribute']]))
					{
						...
						'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
							...
							<td style="padding:0.6em 0.4em;"><strong>'.$product['name'].(isset($product['attributes_small']) ? ' '.$product['attributes_small'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').' - '.$this->l('Customized').(!empty($customizationText) ? ' - '.$customizationText : '').'</strong></td>
					...
					if (!$customizationQuantity OR (int)$product['cart_quantity'] > $customizationQuantity)
						...
						'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
							<td style="padding: 0.6em 0.4em;">'.$product['reference'].'</td>
							<td style="padding:0.6em 0.4em;"><strong>'.$product['name'].(isset($product['attributes_small']) ? ' '.$product['attributes_small'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').'</strong></td>
							<td style="padding: 0.6em 0.4em; text-align: right;">'.Tools::displayPrice(Product::getTaxCalculationMethod() == PS_TAX_EXC ? $price : $price_wt, $currency, false).'</td>
