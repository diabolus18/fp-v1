...
class		Cart extends ObjectModel
{
	...
	public function getProducts($refresh = false, $id_product = false)
	{
		...
		$sql = '
		SELECT cp.`id_product_attribute`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.`instructions` AS instructions, cp.`instructions_valid` AS instructions_valid, cp.`instructions_id` AS instructions_id, pl.`name`,
		...
		foreach ($result AS $k => $row)
		{
			...
			//$row['instructions'] = $row['cart_instructions'];
			//$row['instructions_id'] = $row['cart_instructions_id'];
			//$row['instructions_valid'] = $row['cart_instructions_valid'];
	...
	public function containsProduct($id_product, $id_product_attribute, $id_customization, $instructions)
	{
		return Db::getInstance()->getRow('
			...
			WHERE cp.`id_product` = '.intval($id_product).' AND cp.`id_product_attribute` = '.intval($id_product_attribute).' AND cp.`id_cart` = '.intval($this->id). ' AND (cp.instructions = "'.mysql_real_escape_string($instructions).'" OR cp.instructions_valid = "'.mysql_real_escape_string($instructions).'")'.
	...
	public	function updateQty($quantity, $id_product, $id_product_attribute = NULL, $id_customization = false, $operator = 'up',$instructions, $instructions_id)
	{
		...
		if (intval($quantity) <= 0)
			return $this->deleteProduct(intval($id_product), intval($id_product_attribute), intval($id_customization),$instructions);
		else
		{
			/* Check if the product is already in the cart */
			$result = $this->containsProduct($id_product, $id_product_attribute, $id_customization,$instructions);
			...
			if (Db::getInstance()->NumRows())
			{
				...
				if ($newQty <= 0)
					...
					($id_product_attribute != NULL ? ' AND `id_product_attribute` = '.intval($id_product_attribute) : '').' AND (instructions = "'.$instructions.'" OR  instructions_valid = "'.($instructions?md5($instructions):"").'")'.'
			...
			else
			{
				...
				'id_product_attribute' => intval($id_product_attribute), 'id_cart' => intval($this->id), 'instructions' => $instructions, 'instructions_valid' => ($instructions?md5($instructions):""), 'instructions_id' => $instructions_id,
	...
	*/
	...
	public	function deleteProduct($id_product, $id_product_attribute = NULL, $id_customization = NULL, $instructions)
	{
		...
		if (Db::getInstance()->NumRows() AND intval($result['quantity']))
			return Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'cart_product` SET `quantity` = '.intval($result['quantity']).' WHERE `id_cart` = '.intval($this->id).' AND (instructions = "'.$instructions.'" OR instructions_valid = "'.$instructions.'") AND `id_product` = '.intval($id_product).($id_product_attribute != NULL ? ' AND `id_product_attribute` = '.intval($id_product_attribute) : ''));

		/* Product deletion */
		return Db::getInstance()->Execute('DELETE FROM `'._DB_PREFIX_.'cart_product` WHERE `id_product` = '.intval($id_product).($id_product_attribute != NULL ? ' AND `id_product_attribute` = '.intval($id_product_attribute) : '').' AND (`instructions` = \''.$instructions.'\' OR instructions_valid = "'.$instructions.'") AND `id_cart` = '.intval($this->id));
	}
