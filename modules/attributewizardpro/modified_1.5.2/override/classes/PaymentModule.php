					...
					foreach ($products as $key => $product)
					{
						...
						if (isset($customized_datas[$product['id_product']][$product['id_product_attribute']]))
						{
							...
							'<tr style="background-color: '.($key % 2 ? '#DDE2E6' : '#EBECEE').';">
								...
								<td style="padding: 0.6em 0.4em;"><strong>'.$product['name'].(isset($product['attributes']) ? ' - '.$product['attributes'] : '').(isset($product['instructions']) ? ' '.$product['instructions'] : '').' - '.Tools::displayError('Customized').(!empty($customization_text) ? ' - '.$customization_text : '').'</strong></td>
