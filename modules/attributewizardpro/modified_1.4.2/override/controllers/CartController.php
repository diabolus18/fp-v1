				...
				else
				{
					...
					if (!sizeof($this->errors))
					{
						if ($add AND $qty >= 0)
						{
							...
							if (!sizeof($this->errors))
							{
								$updateQuantity = self::$cart->updateQty((int)($qty), (int)($idProduct), (int)($idProductAttribute), $customizationId, Tools::getValue('op', 'up'),$instructions, $instructions_id);
						...
						elseif ($delete)
						{
							if (self::$cart->deleteProduct((int)($idProduct), (int)($idProductAttribute), (int)($customizationId), $instructions))
