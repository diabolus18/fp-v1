...
class Cart extends CartCore
{
	...
	public	function deleteProduct($id_product, $id_product_attribute = NULL, $id_customization = NULL, $instructions)
	{
		...
		if (($result = Db::getInstance()->getRow('SELECT SUM(`quantity`) quantity FROM `'._DB_PREFIX_.'customization` WHERE `id_cart` = '.(int)$this->id.' AND `id_product` = '.(int)$id_product.' AND `id_product_attribute` = '.(int)$id_product_attribute)) === false && $instructions == '')
			return false;
	...
	public function duplicate()
	{
		...
		foreach ($products as $product)
			$cart->updateQty($product['quantity'], (int)$product['id_product'], $product['instructions_id']!= ''?$awp->getIdProductAttribute($product['id_product'],$product['instructions_id']):(int)$product['id_product_attribute'], NULL, 'up',$product['instructions'], $product['instructions_id']);
