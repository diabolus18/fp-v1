...
if ($add OR Tools::getIsset('update') OR $delete)
{
	...
	$instructions = $ins != "undefined"?$ins:"";
		...
		else
		{
			...
			if (!sizeof($errors))
			{
				if ($add AND $qty >= 0)
				{
					...
					if (!sizeof($errors) AND !$cart->updateQty(intval($qty), intval($idProduct), intval($idProductAttribute), $customizationId, Tools::getValue('op', 'up'),$instructions, $instructions_id))
				...
				elseif ($delete)
				{
					$cart->deleteProduct(intval($idProduct), intval($idProductAttribute), $customizationId, $instructions);
