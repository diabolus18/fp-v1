...
<div style="width: {sizeof($products)*230+190}px;" class="content_comapre">
	...
	<tr>
		...
		{foreach from=$products item=product name=for_products}
			...
			{$i=$i+1}
			<td style="width:230px" class="ajax_block_product comparison_infos{if $i==1} first{/if}{if $i==count($products)} last{/if}">
				...
				<div class="product_desc">{$product->description_short|strip_tags|truncate:100:'...'}</div>			
					...
					{if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
						...
						{if $product->on_sale || ($product->specificPrice AND $product->specificPrice.reduction)}
