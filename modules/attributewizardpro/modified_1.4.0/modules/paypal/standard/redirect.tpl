...
<html>
	...
	<body>
		...
		<form action="{$paypal_url}" method="post" id="paypal_form" class="hidden">
			...
			<input type="hidden" name="item_name_{$k+1}" value="{$product.name|escape:'htmlall':'UTF-8'}{if isset($product.attributes)} - {$product.attributes|escape:'htmlall':'UTF-8'}{/if}{if isset($product.instructions)} - {$product.instructions|strip_tags|escape:'htmlall':'UTF-8'}{/if}" />
