 ...
 {if is_array($data) && $data|@count > 0}
  ...
  {foreach from=$data item=menu key=k name=menu }
    ...
    {if is_array($menu['children']) && $menu['children']|@count > 0}
      <li class="{if $menu['children']}parent{/if}"><a href="{$menu['link']}">
		  {$menu['name']} </a>
...
<div class="cs_mega_menu" id="menu" >
	...
	{foreach from=$menus item=menu  name=menus}
		<li class="menu_item{if $smarty.foreach.menus.first} menu_first{/if} {if $smarty.foreach.menus.last}menu_last {/if}level-1{if $menu->options && $menu->options|@count > 0} parent{/if}">
			{if isset($menu->description[(int)$cookie->id_lang])}<div class="des_menu">{$menu->description[(int)$cookie->id_lang]}</div>{/if}
								...
								{if isset($option['product_list'][$ul])}
									...
									<div class="center_block">
										{if $option['content_option']->opt_show_image_product ==  1 } 											
											<div class="image"><a title="{$product->name|escape:'htmlall':'UTF-8'}" href="{$product->getLink()}">
											<img src="{$link->getImageLink($product->link_rewrite, $product->images['id_image'], $option['content_option']->opt_image_size_product)}" alt=""/>
											</a></div>
										{/if}
										<div class="name_product">
										<h3><a title="{$product->name|escape:'htmlall':'UTF-8'}" href="{$product->getLink()}">{$product->name|escape:'htmlall':'UTF-8'|truncate:35:'...'}</a></h3></div>
