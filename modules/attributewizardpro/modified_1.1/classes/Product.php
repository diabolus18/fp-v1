...
class		Product extends ObjectModel
{
	...
	/** @var float Price Without tax in euros */
	public 		$price_nt = 0;
...
/** @var float Wholesale Price in euros */
	...
	public function getFields()
	{
		...
		$fields['price_nt'] = floatval($this->price_nt);
		//$fields['price_nt'] = floatval($this->price);
	...
	*/
	public static function getPriceStatic($id_product, $usetax = true, $id_product_attribute = NULL, $decimals = 6, $divisor = NULL, $only_reduc = false, $usereduc = true, $quantity = 1)
	{
		...
		$awp_no_tax_impact = intval(Configuration::get('AWP_NO_TAX_IMPACT'));
		if ($awp_no_tax_impact == 1 && isset($result['attribute_price']))
			$price += $result['attribute_price'];
		...
		if ($usetax)
			$price *= (1 + ($tax / 100));
		if ($awp_no_tax_impact == 0 && isset($result['attribute_price']))
	...
	*/
	...
	static public function addCustomizationPrice(&$products, &$customizedDatas)
	{
		foreach ($products AS &$productUpdate)
		{
			...
			{
				$productUpdate['total_wt'] = number_format($priceWt * ($productQuantity - $customizationQuantity), 6, '.', '');
				$productUpdate['total_customization_wt'] = number_format($priceWt * $customizationQuantity, 6, '.', '');
			}
		}
	}
