...
class Cart extends CartCore
{
	...
	public	function updateQty($quantity, $id_product, $id_product_attribute = NULL, $id_customization = false, $operator = 'up',$instructions, $instructions_id)
	{
		...
		else
		{
			...
			fwrite($fh, "include \n\r".dirname(__FILE__)."/../../modules/attributewizardpro/attributewizardpro.php");
			fclose($fh);
						include_once(dirname(__FILE__).'/../../modules/attributewizardpro/attributewizardpro.php');
						$awp = new AttributeWizardPro();
			$myFile = dirname(__FILE__)."/1_log.txt";
			$fh = fopen($myFile, 'a') or die("can't open file");
			fwrite($fh, "width: $awp->_awp_default_group");
			fclose($fh);
						if ($newQty > $productQty || !$awp->checkCartQuantity((int)$id_product, $quantity, $instructions_id))
							return false;
					}
				}
